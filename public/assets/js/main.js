...
// Add an event listener to the "Add to Portfolio" button
document.getElementById('add-to-portfolio-btn').addEventListener('click', function() {
  // Show the modal form
  addPortfolioModal.style.display = 'block';
});

// Add an event listener to the "Add" button in the modal form
// Add an event listener to the "Add" button in the modal form
document.getElementById('add-btn').addEventListener('click', function(event) {
  ...
  const imageSrc = document.getElementById('image-input').value;
  const category = document.getElementById('category-input').value;
  const link = document.getElementById('link-input').value;

  // Add the new portfolio item
  addPortfolioItem(title, imageSrc, category, link);
...
function addPortfolioItem(title, imageSrc, category, link) {
  const newPortfolioItem = document.createElement('div');
  newPortfolioItem.classList.add('portfolio-item', 'filter-app');
  newPortfolioItem.innerHTML = `
    <img src="${imageSrc}" class="img-fluid" alt="">
    <div class="portfolio-info">
      <h4>${title}</h4>
      <p>${category}</p>
      <a href="${imageSrc}" data-gallery="portfolioGallery" class="portfolio-lightbox preview-link" title="${title}"><i class="bx bx-plus"></i></a>
